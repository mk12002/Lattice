...
_FALLBACK_RE = re.compile(
    r"(?<![A-Za-z0-9])(" + "|".join(re.escape(t) for t in sorted(_FALLBACK_TOKENS, key=len, reverse=True)) + r")(?![A-Za-z0-9])",
    re.IGNORECASE,
)


def regex_fallback_scan(
    path: PurePath, content: str, detector_name: str
) -> Iterable[CryptoAsset]:
