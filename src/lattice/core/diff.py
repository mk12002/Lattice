...
class DiffResult:
    ...
    def new_at_or_above(self, threshold: Priority) -> int:
        ...
        return sum(
            ...
            if priority != Priority.NONE.value and Priority(priority).rank <= threshold.rank
        )
...
def render_text(result: DiffResult) -> str:
    ...
    if result.baseline_score is not None and result.current_score is not None:
        arrow = "->"
        lines.append(f"readiness score: {result.baseline_score} {arrow} {result.current_score}")
