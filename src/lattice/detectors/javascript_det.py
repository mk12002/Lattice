...
_GENERATE_KEYPAIR = re.compile(r"\bgenerateKeyPair(?:Sync)?\s*\(\s*[\"']([a-zA-Z0-9-]+)[\"']")
...
_IMPORT = re.compile(r"(?:require\s*\(\s*[\"']([^\"']+)[\"']\s*\)|from\s+[\"']([^\"']+)[\"'])")
            ...
            for m in _IMPORT.finditer(line_text):
                ...
                if package in _CRYPTO_PACKAGES:
                    ...
                    if package_algorithm:
                        yield from emit(i, package_algorithm, note=f"imported package '{package}'")
                    else:
                        yield from emit(
                            i,
                            package,
