...
def _component(finding: Finding) -> dict:
    ...
    if assessment.pqc_replacement:
        properties.append({"name": "lattice:pqcReplacement", "value": assessment.pqc_replacement})
    if finding.accepted_reason is not None:
        properties.append({"name": "lattice:accepted", "value": "true"})
        properties.append({"name": "lattice:acceptedReason", "value": finding.accepted_reason})
    if asset.snippet:
