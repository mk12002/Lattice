...
1. **Create `src/lattice/detectors/<lang>_det.py`** implementing the ABC in
   [`src/lattice/detectors/base.py`](src/lattice/detectors/base.py):
...
2. **Register it** in `src/lattice/detectors/registry.py` (`LANGUAGE_MAP`).
...
1. Add the entry in `src/lattice/rules/algorithms.py` with `family`, `quantum_status`,
